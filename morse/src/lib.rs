//! Morse code timing and transmission for an Arduino UNO R3 LED.
...
fn transmit_morse<L: OutputPin>(led: &mut L, morse: &str) {
    for (i, symbol) in morse.chars().enumerate() {
        match symbol {
            '.' => {
                led.set_high().ok();
                delay_ms(DIT_DURATION_MILLIS);
                led.set_low().ok();
            }
            '-' => {
                led.set_high().ok();
                delay_ms(DAH_DURATION_MILLIS);
                led.set_low().ok();
            }
            _ => {}
        }
        ...
        if i < morse.len() - 1 {
            delay_ms(INTRA_CHARACTER_GAP_MILLIS);
        }
    }
}

/// Converts and displays one supported letter or digit.
fn transmit_character<L: OutputPin>(led: &mut L, c: char) {
    if let Some(morse) = char_to_morse(c) {
        transmit_morse(led, morse);
    }
}

/// Displays a string in Morse code, using spaces as word separators.
pub fn transmit_string<L: OutputPin>(led: &mut L, message: &str) {
